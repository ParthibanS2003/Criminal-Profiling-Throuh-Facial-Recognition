...
st.title("children Face Recognition system App")
...
if choice == "Adding":
            ...
            if submit_btn:
                if name == "" or id == "":
                    ...
                    if ret == 1: 
                        ...
                        st.error("Children ID already exists")
                    elif ret == -1: 
    ...
    if submit_btn:
        ...
        if name == None and image == None:
            st.error("Children ID does not exist")
        else:
            st.success(f"Name of Children with ID {id} is: {name}")
            st.warning("Please check the image below to make sure you are deleting the right Children")
            st.image(image)
    ...
    if submit_btn:
        ...
        if old_name == None and old_image == None:
            ...
            with st.form(key='my_form'):
                st.title("Adjusting Children info")
